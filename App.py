...
def run():
    ...
    activities = ["Image", "Webcam"] # هنا بقول بقدر اختار صورة او كام
    ...
    if choice == 'Image':
        ...
        img_file = st.file_uploader("Choose an Image", type=['jpg', 'jpeg', 'jfif', 'png'])# هنا اعمل file_uploader
        if img_file is not None:
            img = np.array(Image.open(img_file))
            img1 = cv2.resize(img, (350, 350))
            place_h = st.columns(2)
            place_h[0].image(img1) # علشان اعرضها 
            st.markdown(
